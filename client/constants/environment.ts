export const SITE_NAME = 'Asciify';
export const SITE_TITLE = 'Asciify - Word Art';
export const SITE_DESCRIPTION = 'Generate beautiful ASCII art from images and make your website stand out easily. Supports multiple image types and can be saved as a text file or an html';
export const SITE_KEYWORDS = 'ascii,word art,art,ascii image,wallpaper,coding,developer,terminal,website'
export const SITE_IMAGE = '/favicon.png';

export const GA_TRACKING_ID = 'UA-165896975-1';

export const ENABLE_ACCOUNTS = false;

export const DEV = process.env.NODE_ENV !== 'production';

export const CORS_ANYWHERE = 'https://ukiyo-cors-anywhere.herokuapp.com/';