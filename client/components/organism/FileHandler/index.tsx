  ...
  async function handleOnUrlSubmit(e: FormEvent) {
    ...
    try {
      const { data } = await axios({
        method: 'get',
        url: CORS_ANYWHERE + urlInputValue,
      ...
      const imageData = {
        data: imageBase64,
        name: urlInputValue.split(/\//gi).pop(),
        size: new Buffer(imageBase64, 'base64').length
      };
          ...
          <S.PrimaryContent>
            <S.UrlWrapper onSubmit={handleOnUrlSubmit}>
              <S.UrlInput
                ref={urlInputRef}
                bordered
                placeholder={FILE_HANDLER.urlInputPlaceholder}
                aria-label="Url Input"
              />
              <S.UrlButton submitButton loading={loading} disabled={loading}>
                {FILE_HANDLER.loading}
              </S.UrlButton>
            </S.UrlWrapper>
